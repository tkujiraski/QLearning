...
class Persuit2:
    ...
    def __init__(self, erate, view):
        ...
        self.ysize = 7
        self.xsize = 7
        self.ql = QLearning.QLearning([9], 5, 0.05, 0.8, 0.1, 1500, 50000, self.inif, self.act, self.checkg)
    ...
    def inif(self):
        ...
        while True:
            ...
            if self.target != self.hunter:
                break
        self.target_loc  = [self.target.copy()]
        # 獲物の見えている方向を決定
        dir = self.direction(self.hunter, self.target, self.view)
        return [dir]
    ...
    def act(self, s, a):
        """状態sでアクションaを行った場合の次の状態にsを書き換え、報酬を返す"""
         # 獲物の行動
        ta = randint(5)
        ng = [self.hunter] # ハンターの位置には移動できない。実際にはハンターの位置に移動できる場合、すでに捕まっている
        err = np.random.rand()
        if err > self.erate:
        ...
        self.hunter, cond = self.move(self.hunter, a, ng)
        dir = self.direction(self.hunter, self.target, self.view)
        # 状態更新
        s[0] = dir
        if cond == 1:
    ...
    def checkg(self,s):
        """stateが終端状態がどうかを判定する"""
        #上下左右に獲物がいれば終了
        dy = abs(self.hunter[0] - self.target[0])
        dx = abs(self.hunter[1] - self.target[1])
